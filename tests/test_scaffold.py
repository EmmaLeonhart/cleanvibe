...
class TestCreateProject(unittest.TestCase):
    ...
    def test_claude_md_cron_lifecycle_mentions_start(self):
        # The CLAUDE.md cron lifecycle must say to START the crons at the
        # beginning of extensive work — not only kill/restart them. v1.11.0
        # generalized this from a single status-report cron to the three-cron
        # playbook (work-loop / auto-flush / status-report).
    ...
    def test_claude_md_has_weekly_update_check_section(self):
        # v1.11.0: every generated CLAUDE.md carries a self-update pointer so
        # long-lived projects can pick up new cleanvibe skills without being
        # re-scaffolded. Section names the canonical updates URL, the
        # generating cleanvibe version, and the last-check date.
        content = templates.claude_md("myproj")
        lower = content.lower()
        self.assertIn("check cleanvibe for skill updates", lower)
        self.assertIn("https://cleanvibe.emmaleonhart.com/updates.md", content)
        self.assertIn("generated by cleanvibe version", lower)
