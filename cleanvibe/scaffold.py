...
def clone_project(repo: str, path: Path, dry_run: bool = False, no_claude: bool = False) -> None:
    ...
    if not devlog.exists():
        _write(devlog, templates.devlog_md(project_name, clone=True))
        print(f"  Injected devlog.md (was missing; backfill from git log per onboarding queue)")
...
def convert_project(path: Path, dry_run: bool = False, no_claude: bool = False) -> None:
    ...
    if not is_git_repo and injected:
        ...
        scaffold_message = (
            ...
            f"missing CLAUDE.md / queue.md / devlog.md / README.md / .gitignore as applicable."
        )
        subprocess.run(
...
def _inject_scaffold(path: Path, project_name: str, is_windows: bool) -> bool:
    ...
    devlog = path / "devlog.md"
    if not devlog.exists():
        # Convert is an existing directory — use the clone-style starter so
        # the agent knows to backfill from git log (if any).
        _write(devlog, templates.devlog_md(project_name, clone=True))
        print(f"  Injected devlog.md (was missing)")
        injected = True
...
def _git_init(path: Path, message=None) -> None:
    ...
    if message is None:
        message = (
            ...
            f"- queue.md (active work queue; delete-only)\n"
