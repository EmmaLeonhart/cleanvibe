...
- **Automated safety scan of cloned/recipe code before running it.** Replication runs third-party code (the authors' recipe, cloned repos, downloaded zips). v1.6.1 added a **consent gate** — the generated `queue.md`/`SKILL.md` make the agent stop and get explicit user consent before executing any such code. The longer-horizon enhancement is an automated, lightweight **security scan** of that code (e.g. flag obvious exfiltration / destructive / network-callout patterns, surface a risk summary) so the consent decision is informed rather than blind. Until then the gate is "ask the user; only replicate trusted sources."
...
The refined structure has landed: `replicating-<slug>` auto-naming (with `-2/-3` collision suffix), the paper at `replication_target/paper.pdf` (gitignored, **not** in `data_lake/`), `data_lake/` still present for other downloaded material, and scaffolded GitHub Actions workflows (Pages + ZIP-package) with user-configured TODOs. **v1.3.0** hardened link parsing (any arXiv/alphaxiv URL path, incl. alphaxiv's primary `/overview/`, resolves — previously only `abs|pdf|html`) and added **manual drop-in mode** (`cleanvibe replicate <folder>` for papers you supply by hand: no fetch, non-destructive injection). **v1.4.0** went further: the arXiv **DOI** form and `arXiv:<id>` style now parse, the `vN` **version** is preserved (not dropped), `fetch_paper` is **429-resilient** (Retry-After-aware backoff), the generated `download_paper.py` fetches the **HTML first** (better than PDF for structured text), and the replicate queue/SKILL now tell the agent to **follow the authors' replication recipe first**. The extractor/report/site programs above are the deep follow-on work.
